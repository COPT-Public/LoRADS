...
asdp_retcode AReadSDPA( char *fname, int *pnConstrs, int *pnBlks, int **pblkDims, double **prowRHS,
  ...
  ++whichLine;
  ASDP_INIT(coneDims, int, nCones);
  ASDP_MEMCHECK(coneDims);
  ...
  read_line(file, fileLine, whichLine);
  ASDP_INIT(rowRHS, double, nConstrs);
  ASDP_MEMCHECK(rowRHS);
  ...
  if ( nLPCols > 0 ) {
      ...
      ASDP_MEMCHECK(LpConeData);
  }
  
  ASDP_INIT(pSDPConeData, dcs *, nCones);
  ASDP_MEMCHECK(pSDPConeData);
  
  for ( iCone = 0; iCone < nCones; ++iCone ) {
      pSDPConeData[iCone] = dcs_spalloc(PACK_NNZ(coneDims[iCone]), nConstrs + 1, 100, 1, 1);
      ASDP_MEMCHECK(pSDPConeData[iCone]);
  }
  ...
  /* Copy the data out */
  ASDP_INIT(coneMatBeg, int *, nCones);
  ASDP_INIT(coneMatIdx, int *, nCones);
  ASDP_INIT(coneMatElem, double *, nCones);
  
  ASDP_MEMCHECK(coneMatBeg);
  ASDP_MEMCHECK(coneMatIdx);
  ASDP_MEMCHECK(coneMatElem);
  
  for ( iCone = 0; iCone < nCones; ++iCone ) {
      
      cscMat = pSDPConeData[iCone];
      ASDP_INIT(coneMatBeg[iCone], int, cscMat->n + 1);
      ASDP_INIT(coneMatIdx[iCone], int, cscMat->p[cscMat->n]);
      ASDP_INIT(coneMatElem[iCone], double, cscMat->p[cscMat->n]);
      
      ASDP_MEMCHECK(coneMatBeg[iCone]);
      ASDP_MEMCHECK(coneMatIdx[iCone]);
      ASDP_MEMCHECK(coneMatElem[iCone]);
      
      ASDP_MEMCPY(coneMatBeg[iCone], cscMat->p, int, cscMat->n + 1);
      ASDP_MEMCPY(coneMatIdx[iCone], cscMat->i, int, cscMat->p[cscMat->n]);
      ASDP_MEMCPY(coneMatElem[iCone], cscMat->x, double, cscMat->p[cscMat->n]);
      
  }
  
  if ( nLPCols > 0 ) {
      
      ASDP_INIT(LpMatBeg, int, LpConeData->n + 1);
      ASDP_INIT(LpMatIdx, int, LpConeData->p[LpConeData->n]);
      ASDP_INIT(LpMatElem, double, LpConeData->p[LpConeData->n]);
      
      ASDP_MEMCHECK(LpMatBeg);
      ASDP_MEMCHECK(LpMatIdx);
      ASDP_MEMCHECK(LpMatElem);
      
      ASDP_MEMCPY(LpMatBeg, LpConeData->p, int, LpConeData->n + 1);
      ASDP_MEMCPY(LpMatIdx, LpConeData->i, int, LpConeData->p[cscMat->n]);
      ASDP_MEMCPY(LpMatElem, LpConeData->x, double, LpConeData->p[cscMat->n]);
  }
...
exit_cleanup:
  ...
  if ( retcode != ASDP_RETCODE_OK ) {
      ...
      if ( coneMatBeg ) {
          for ( iCone = 0; iCone < nCones; ++iCone ) {
              ASDP_FREE(coneMatBeg[iCone]);
          }
          ASDP_FREE(coneMatBeg);
      }
      
      if ( coneMatIdx ) {
          for ( iCone = 0; iCone < nCones; ++iCone ) {
              ASDP_FREE(coneMatIdx[iCone]);
          }
          ASDP_FREE(coneMatIdx);
      }
      
      if ( coneMatElem ) {
          for ( iCone = 0; iCone < nCones; ++iCone ) {
              ASDP_FREE(coneMatElem[iCone]);
          }
          ASDP_FREE(coneMatElem);
      }
