...
//{
//    double *gradTemp;
//    ASDP_INIT(gradTemp, double, ASolver->nRows);
//    double sa = 2.0;
//    int incx = 1;
//    for (int iCone = 0; iCone < ASolver->nCones; ++iCone)
//    {
...
//        Cone->sdp_slack_var->zeros(Cone->sdp_slack_var->dataMat);
//        Cone->addObjCoeff(Cone->coneData, Cone->sdp_slack_var);
//        Cone->sdpDataWSum(Cone->coneData, lambda, Cone->sdp_slack_var);
//        if (Cone->sdp_slack_var->dataType == SDP_COEFF_DENSE)
//        {
//            sdp_coeff_dense *dense = (sdp_coeff_dense *)Cone->sdp_slack_var->dataMat;
//            int n = dense->nSDPCol * (dense->nSDPCol + 1) / 2;
//            scal(&n, &sa, dense->dsMatElem, &incx);
//        }
//        else if (Cone->sdp_slack_var->dataType == SDP_COEFF_SPARSE)
//        {
//            sdp_coeff_sparse *sparse = (sdp_coeff_sparse *)Cone->sdp_slack_var->dataMat;
//            scal(&sparse->nTriMatElem, &sa, sparse->triMatElem, &incx);
//        }
//        Cone->coneAUV(Cone->coneData, Udummy[iCone], Vdummy[iCone], gradTemp, Cone->sdp_slack_var, FLAG_UVt);
//        double alpha = 1.0;
//        axpy(&ASolver->nRows, &alpha, gradTemp, &incx, grad, &incx);
//    }
//
//    double pDualObj = dot(&ASolver->nRows, ASolver->rowRHS, &incx, lambda, &incx);
//    double pdGap = pDualObj - ASolver->pObjVal;
//    axpy(&ASolver->nRows, &pdGap, ASolver->rowRHS, &incx, grad, &incx);
//}
//
...
//{
//    ASDP_ZERO(grad, double, ASolver->nRows);
//    asdpLPCone *lpCone = ASolver->lpCone;
//
//    double *weight;
//    ASDP_INIT(weight, double, lpCone->nCol);
//    ASDP_ZERO(weight, double, lpCone->nCol);
//    double alpha = 1.0;
//    int incx = 1;
//    asdp_cone_lp *coneLpData = (asdp_cone_lp *)lpCone->coneData;
//    axpy(&lpCone->nCol, &alpha, coneLpData->objMatElem, &incx, weight, &incx);
//
//    for (int iCol = 0; iCol < lpCone->nCol; ++iCol)
//    {
//        lpCone->lpDataWSum(lpCone->coneData, lambda, &weight[iCol], iCol);
//    }
//
//    lpCone->coneAUV2(lpCone->coneData, weight, grad);
//    dualGradStandardSDP(ASolver, lambda, grad, Udummy, Vdummy);
//    ASDP_FREE(weight);
//}
//
...
//    {
//        double *CAlambdaV;
//        ASDP_INIT(CAlambdaV, double, V[iCone]->nRows *V[iCone]->rank);
//        shell->nRows = V[iCone]->nRows;
//        shell->rank = V[iCone]->rank;
//        asdp_cone *ACone = ASolver->ACones[iCone];
//        ACone->sdp_obj_sum->zeros(ACone->sdp_obj_sum->dataMat);
//        ACone->addObjCoeff(ACone->coneData, ACone->sdp_obj_sum);
//        ACone->sdpDataWSum(ACone->coneData, lambda, ACone->sdp_obj_sum);
//        ACone->sdp_obj_sum->mul_rk(ACone->sdp_obj_sum->dataMat, V[iCone], CAlambdaV);
//        shell->matElem = CAlambdaV;
//
//        ASDPUVt(ACone->sdp_obj_sum, shell, V[iCone]);
//        double *val;
//        valRes(ASolver->constrVal[iCone]->constrVal, ASolver->constrVal[iCone]->type, &val);
//        ACone->coneAUV(ACone->coneData, shell, V[iCone], val, ACone->sdp_obj_sum, FLAG_UVt);
//
//        // axpy(&ASolver->nRows, &alpha, ASolver->constrVal[iCone], &incx, grad, &incx);
//        ASolver->constrVal[iCone]->add(&alpha, ASolver->constrVal[iCone]->constrVal, grad);
//        ASDP_FREE(CAlambdaV);
//    }
//    ASDP_FREE(shell);
//}
...
//    ASDP_ZERO(grad, double, ASolver->nRows);
//    asdpLPCone *lpCone = ASolver->lpCone;
//    asdp_rk_mat_lp *uOrv = ASolver->rLp;
//    double *weight;
//    ASDP_INIT(weight, double, lpCone->nCol);
//    ASDP_ZERO(weight, double, lpCone->nCol);
//    double alpha = 1.0;
...
//    }
//    // calculate twice
//    vvscl(&lpCone->nCol, uOrv->matElem, weight);
//    vvscl(&lpCone->nCol, uOrv->matElem, weight);
//    lpCone->coneAUV2(lpCone->coneData, weight, grad);
//
//    dualGradNoncvxSDP(ASolver, lambda, grad, Udummy, V);
//
//    ASDP_FREE(weight);
//}
