...
void *dcs_malloc (int n, size_t size) {
    ...
    return (malloc (ASDP_MAX (n, 1) * size)) ;
}

/* wrapper for calloc */
void *dcs_calloc (int n, size_t size) {
    
    return (calloc (ASDP_MAX (n,1), size)) ;
}
...
void *dcs_realloc (void *p, int n, size_t size, int *ok) {
    ...
    void *pnew ;
    pnew = realloc (p, ASDP_MAX (n,1) * size) ; /* realloc the block */
...
dcs *dcs_spalloc (int m, int n, int nzmax, int values, int triplet) {
    ...
    A->n = n ;
    A->nzmax = nzmax = ASDP_MAX (nzmax, 1) ;
...
int dcs_entry (dcs *T, int i, int j, double x) {
    ...
    T->i [T->nz] = i ;
    T->p [T->nz++] = j ;
    T->m = ASDP_MAX (T->m, i+1) ;
    T->n = ASDP_MAX (T->n, j+1) ;
    return (1) ;
}
